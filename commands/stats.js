...
module.exports = {
    ...
    execute(message, args) {
        ...
        .end((error, response) => {
            if(error) {
                logger.logWarn(`Error parsing hiscores for user: ${error.status || error.response}`);
                message.channel.send(`there was an issue looking up stats for ${args}. Please try again later.`);
            } else {
